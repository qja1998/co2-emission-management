...
class CarbonPredictionView(APIView):
    ...
    def get(self, request, depart_name, format=None): #request.GET[' ']으로 얻을 수 있음
        ...
        carbon_info = CarbonInfo.objects.filter(EndtDate=CUR_TIME, Chief=Root.RootCom.Chief)
        cur_data = Carbon.objects.filter(RootCom=Root.RootCom,
                                         BelongDepart=Root.BelongCom,
                                         CarbonInfo=carbon_info
                                         ).values('CarbonData')
        predictions = CarbonPrediction.objects.filter(Com_id=Root.RootCom.pk, PredictDate=AFTER_MON)
